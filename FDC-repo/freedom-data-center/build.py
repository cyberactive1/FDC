...
import base64, mimetypes, os, pathlib, re, json, shutil, sys
...
# Values the source cannot know. All are substituted into placeholders in the
# .dc.html, and the build asserts afterwards that no placeholder survived (a
# stale __SITE_URL__ inside a canonical tag is invisible until a crawler reads
# it).
#
#   FDC_SITE_URL          public origin; canonical, og:url, og:image, JSON-LD,
#                         sitemap. Set empty to strip every URL-bearing line and
#                         keep the artifact totally URL-free.
#   FDC_CONTACT_ENDPOINT  URL accepting a JSON POST from the two forms. Empty
#                         means they confirm on-page and deliver NOWHERE.
#
# On Vercel both are inferred when unset, because hardcoding an origin breaks
# preview deployments -- every branch gets its own hostname, and a preview that
# advertises the production canonical asks Google to index the wrong build.
PLACEHOLDER_ORIGIN = 'https://freedomdatacenter.com'

VERCEL_ENV  = os.environ.get('VERCEL_ENV', '')          # production|preview|development
ON_VERCEL   = bool(os.environ.get('VERCEL'))
IS_PREVIEW  = ON_VERCEL and VERCEL_ENV != 'production'


def resolve_site_url():
    explicit = os.environ.get('FDC_SITE_URL')
    if explicit is not None:                            # set, possibly to ''
        return explicit.rstrip('/'), 'FDC_SITE_URL'
    # Vercel exposes the stable production hostname and the per-deployment one.
    host = (os.environ.get('VERCEL_URL') if IS_PREVIEW
            else os.environ.get('VERCEL_PROJECT_PRODUCTION_URL')
                 or os.environ.get('VERCEL_URL'))
    if host:
        return 'https://' + host.rstrip('/'), 'Vercel'
    return PLACEHOLDER_ORIGIN, 'fallback'


def resolve_contact_endpoint():
    explicit = os.environ.get('FDC_CONTACT_ENDPOINT')
    if explicit is not None:
        return explicit.strip()
    # api/inquiries.js ships in this repo; same-origin, so no CORS to configure.
    return '/api/inquiries' if ON_VERCEL else ''


SITE_URL, SITE_URL_SOURCE = resolve_site_url()
CONTACT_ENDPOINT = resolve_contact_endpoint()
...
# A preview deployment must never be indexed: it would compete with production
# for the same content. Swap the robots directive rather than adding a second.
if IS_PREVIEW:
    before = html.count('<meta name="robots" content="index, follow, max-image-preview:large">')
    assert before == 1, 'robots meta not found for preview rewrite'
    html = html.replace('<meta name="robots" content="index, follow, max-image-preview:large">',
                        '<meta name="robots" content="noindex, nofollow">')

print('environment       %s' % (VERCEL_ENV or 'local'))
print('site url          %s%s' % (SITE_URL or '(none — URL-free build)',
                                  ' [%s]' % SITE_URL_SOURCE if SITE_URL else ''))
print('contact endpoint  %s' % (CONTACT_ENDPOINT or '(none — forms deliver nowhere)'))
print('indexable         %s' % ('no (preview)' if IS_PREVIEW else 'yes'))
...
# Wipe the deploy directory first. robots.txt and sitemap.xml are conditional on
# origin and environment, so a preview build after a production build inherited a
# stale sitemap.xml advertising the production URL -- observed, not theoretical.
...
# ---- 3b. deploy directory ----------------------------------------------------
# Static hosts serve dist/ as the document root, so the same bytes are written
# twice: index.html is what Vercel serves, the named copy is what a person
# downloads and double-clicks.
INDEX = OUT.parent / 'index.html'
INDEX.write_text(html, encoding='utf-8')

# public/ is copied verbatim -- it holds the social card and anything else that
# must be reachable by URL. Copying needs no third-party module, which keeps the
# "no dependencies beyond Python 3" promise; tools/make-og.py regenerates the
# card and is the only script that wants Pillow.
PUBLIC = ROOT / 'public'
copied = []
if PUBLIC.is_dir():
...
# robots.txt / sitemap.xml are generated, never committed: both depend on the
# origin, and a preview deployment must forbid everything.
if IS_PREVIEW or not SITE_URL:
    robots = 'User-agent: *\nDisallow: /\n'
else:
    robots = ('User-agent: *\nAllow: /\n\nSitemap: %s/sitemap.xml\n' % SITE_URL)
(OUT.parent / 'robots.txt').write_text(robots, encoding='utf-8')

if SITE_URL and not IS_PREVIEW:
...
print('\ndeploy dir        %s' % OUT.parent)
print('  index.html, freedom-data-center.html, robots.txt'
      + (', sitemap.xml' if SITE_URL and not IS_PREVIEW else '')
...
# Reveals hide text synchronously and undo it from an IntersectionObserver
# callback. When a callback did not arrive, headings stayed at opacity 0 with
# clip-path: inset(0 100% 0 0) -- invisible text in reserved space. The sweep is
# the guarantee that anything past the trigger line becomes visible regardless.
checks.append((True,  'revealNow(',          'reveal backstop present'))
checks.append((True,  'bootNow(',            'data-boot backstop present'))

# data-m-trim once hid 2-6 items of real copy per run on phones with no way to
# reach them. If a display:none trim rule comes back, it must be a deliberate
# decision about a specific run, not a blanket one.
checks.append((False, '[data-m-trim="10"] > *:nth-child(n+11) { display: none', 'no blanket mobile content truncation'))
checks.append((False, '[data-m-hide] { display: none', 'mobile sub-labels wrap rather than disappear'))
checks.append((True,  '[data-reveal] { opacity: 1 !important', 'reduced-motion guard restores opacity'))
...
warn = []
if SITE_URL == PLACEHOLDER_ORIGIN and SITE_URL_SOURCE == 'fallback':
    warn.append('no origin configured, so the placeholder %s was used --\n'
                '        canonical, og:url and og:image all point at a domain you may not\n'
                '        own. Set FDC_SITE_URL, or deploy on Vercel where it is inferred.'
                % PLACEHOLDER_ORIGIN)
...
# The assertions are worthless unless a failure fails the process: Vercel marks
# the deployment ready on exit 0, and a GitHub check goes green, no matter how
# many FAIL lines scrolled past.
if bad:
    print('\n%d check(s) FAILED — output is wrong, refusing to exit clean.' % bad)
    sys.exit(1)
