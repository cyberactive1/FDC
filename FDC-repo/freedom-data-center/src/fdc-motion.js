...
export function initMotion(root, opts) {
  ...
  const revealNow = el => {
    ...
    if (el.hasAttribute('data-k')) el.style.letterSpacing = '';
  ...
  // ---------- reveal backstop ----------
  // The observer is the mechanism; this is the guarantee. Hiding happens in one
  // synchronous pass above, so anything that does not receive a callback stays
  // at opacity 0 forever -- invisible text in correctly reserved space, which is
  // exactly the failure that principle 6 (fail visible, never hidden) forbids.
  // Headings are the most conspicuous casualty because they also carry
  // clip-path: inset(0 100% 0 0), so a stuck heading is doubly invisible while a
  // stuck paragraph is merely transparent.
  //
  // Sweep anything that has reached the same trigger line the observer uses, so
  // choreography is unchanged; the sweep only ever reveals what the observer
  // should already have revealed. It also covers a load that begins already
  // scrolled, where content above the viewport never intersects at all.
  const TRIGGER = () => window.innerHeight * 0.88;   // matches rootMargin -12%
  let sweepTimer = 0;
  const sweep = () => {
    let pending = 0;
    ...
    if (!pending) stopSweep();
  };
  const stopSweep = () => {
    window.removeEventListener('scroll', sweep);
    if (sweepTimer) { clearInterval(sweepTimer); sweepTimer = 0; }
  };
  sweepTimer = setInterval(sweep, 400);
  window.addEventListener('scroll', sweep, { passive: true });
  cleanups.push(stopSweep);
  sweep();
  ...
  if (bootGroups.length) {
    const bootPanels = g => Array.from(g.children);
    const bootNow = g => {
      bootPanels(g).forEach(panel => {
        panel.style.opacity = '1';
        panel.style.transform = 'none';
      });
      g.setAttribute('data-booted', '');
    };
    ...
    // Same guarantee as the reveal sweep above, and for the same reason: the
    // panels are hidden in one synchronous pass, so a callback that never
    // arrives leaves whole console panels invisible inside a correctly sized
    // grid. These groups are the ADCR metrics and the command-centre tiles --
    // content, not chrome -- so they must not depend on a notification.
    let bootTimer = 0;
    const bootSweep = () => {
      let pending = 0;
      for (const g of bootGroups) {
        if (g.hasAttribute('data-booted')) continue;
        if (g.getBoundingClientRect().top < window.innerHeight * 0.92) {
          bootNow(g);
          bootObs.unobserve(g);
        } else pending++;
      }
      if (!pending) stopBootSweep();
    };
    const stopBootSweep = () => {
      window.removeEventListener('scroll', bootSweep);
      if (bootTimer) { clearInterval(bootTimer); bootTimer = 0; }
    };
    bootTimer = setInterval(bootSweep, 400);
    window.addEventListener('scroll', bootSweep, { passive: true });
    cleanups.push(stopBootSweep);
    bootSweep();
  }
